...
if [ -f "$CERT_DIR/fullchain.cer" ] && [ -f "$CERT_DIR/${CERT_DOMAIN}.key" ]; then
    ...
    if [ $DAYS_LEFT -gt 30 ]; then
        # 证书仍然有效，直接返回
        CERT_CONTENT=$(cat "$CERT_DIR/fullchain.cer" | sed 's/"/\\"/g' | sed ':a;N;$!ba;s/\n/\\n/g')
        
        # 转换为 PKCS#8 格式（.NET 兼容）
        KEY_PKCS8=$(openssl pkcs8 -topk8 -nocrypt -in "$CERT_DIR/${CERT_DOMAIN}.key" 2>/dev/null | sed 's/"/\\"/g' | sed ':a;N;$!ba;s/\n/\\n/g')
...
# 获取证书信息
ISSUER=$(openssl x509 -in "$CERT_DIR/fullchain.cer" -noout -issuer | sed 's/issuer=//' | sed 's/"/\\"/g')
